!!!! getcambpkneu.f   Time-stamp: <2018-10-31 16:04:19 hamana>

! based on getpkmattercamb.F90 -> getcambpklin.f

! gfortran -O3 -fopenmp -ffast-math -fmax-errors=4 -march=native -JRelease -IRelease/  -c getcambpkneu.f -o Release/getcambpkneu.o

      module getmatterpkneucamb

      contains
      
      subroutine getcambpkneu(par_s8,par_ts100,par_h0,par_oc,par_ob,
     $     par_ov,par_on,par_as,par_ns,par_tu,par_ok,par_om,
     $     nkdim,nkmax,ia,pkredshifts,cambpk)
     ...
     $     par_as,par_ns,par_tu,par_ok,par_om
      ...
      integer nkdim,nkmax,ia
      real*8 pkredshifts(ia)
      real cambpk(nkmax,0:ia)
      real*8 ch03
...
!      logical bad
      ...
      ch03=(1.0/2997.92458)**3
      
!      Ini_fail_on_not_found = .false.

!      outroot ='getcambpkneu'      !!!Ini_Read_String('output_root')      
     ...
     $     'HighLExtrapTemplate_lenspotentialCls.dat' !!! Ini_Read_String_Default('highL_unlensed_cl_template',highL_unlensed_cl_template)
      ...
      P%NonLinear =0            !!! Ini_Read_Int('do_nonlinear',NonLinear_none)
...
!!!!!!!!!!!!!!!! need set-up begin !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! TH20181030 modified lin->neu !!!

      if (P%PK_WantTransfer)  then
         P%WantTransfer  = .true.
         P%transfer%kmax          =10.0d0 !!!  Ini_Read_Double('transfer_kmax')
         P%transfer%k_per_logint  =0 !!!  Ini_Read_Int('transfer_k_per_logint')
!!! TH20181030 modified lin->neu !!!         P%transfer%PK_num_redshifts =1 !!!  Ini_Read_Int('transfer_num_redshifts')
         P%transfer%PK_num_redshifts =ia+1 !!!  Ini_Read_Int('transfer_num_redshifts')
...
!!!         P%transfer%PK_redshifts(1)=0.0d0
         do i=1,ia
            P%transfer%PK_redshifts(i)=pkredshifts(i)
!            write(*,*)P%transfer%PK_redshifts(i)
         enddo
         P%transfer%PK_redshifts(ia+1)=0.0d0
!!!!!!!!!!!!!! need input-scheme for redshifs !!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!      do i=1,ia
!         write(*,*)i,P%transfer%PK_redshifts(i),pkredshifts(i)
!      enddo
!      stop
...
!         call Transfer_SaveMatterPower(MT,MatterPowerFileNames)
	 call Transfer_GetMatterPowerDim(MT) !!! define pkmatterdim and pkredshiftsdim
!         write(*,*)pkmatterdim,pkredshiftsdim
         pkredshiftsdim=pkredshiftsdim !!! [defined in Transfer_GetMatterPowerDim of modules.f90] pkredshiftsdim=CP%Transfer%PK_num_redshifts
	 allocate(keepmatterpk(pkmatterdim,0:pkredshiftsdim))
         call Transfer_KeepMatterPower(MT,keepmatterpk)	 !!! compute P(k) and store in keepmatterpk
...
!     enddo
      ...
      nkdim=pkmatterdim
      do j=1,nkdim            
         cambpk(j,0)=keepmatterpk(j,0)
      enddo
      do i=1,ia
         do j=1,nkdim 
            cambpk(j,i)=ch03*keepmatterpk(j,i)
         enddo
      enddo
...
!      write(*,'("# sigma8 at z=0       = ",f9.6)') par_s8
 ...
 100  stop 'Must give num_massive number 
     $     of integer physical neutrinos for each eigenstate'
      end subroutine getcambpkneu

      end module getmatterpkneucamb
