!!!! getcambpklin.f   Time-stamp: <2018-02-28 14:39:16 hamana>

! based on getpkmattercamb.F90

! gfortran -O3 -fopenmp -ffast-math -fmax-errors=4 -march=native -JRelease -IRelease/  -c getcambpklin.f -o Release/getcambpklin.o

      module getmatterpklincamb

      contains
      
      subroutine getcambpklin(par_s8,par_ts100,par_h0,par_oc,par_ob,
...
!      outroot ='getcambpklin'      !!!Ini_Read_String('output_root')      
...
!!!!!!!!!!!!!!!! need set-up begin !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      if (P%PK_WantTransfer)  then
         ...
         P%transfer%PK_num_redshifts =1 !!!  Ini_Read_Int('transfer_num_redshifts')
...
!     enddo
      ...
      do j=1,nkdim            
         ...
         cambpk(j,1)=ch03*keepmatterpk(j,1)
      enddo
...
c      write(*,'("# sigma8 at z=0       = ",f9.6)') par_s8
 ...
 100  stop 'Must give num_massive number 
     $     of integer physical neutrinos for each eigenstate'
      end subroutine getcambpklin

      end module getmatterpklincamb
